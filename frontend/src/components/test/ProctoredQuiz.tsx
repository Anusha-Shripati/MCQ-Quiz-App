...
export default function ProctoredQuiz({ accessCode }: ProctoredQuizProps) {
	...
	const handleVisibilityChange = () => {
		if (document.hidden) {
			...
			displayAlert(
				'WARNING: Tab switching detected! This violation has been recorded.'
			);
	...
	const handleResize = () => {
		...
		if (
			Math.abs(window.innerWidth - width) > 20 ||
			Math.abs(window.innerHeight - height) > 20
		) {
			addViolation({
				type: 'WINDOW_RESIZE',
	...
	useEffect(() => {
		const validateAccess = async () => {
			try {
				...
				setAccessError(
					'Error accessing exam. Please try again or contact support.'
				);
				setIsLoading(false);
			}
		};
	...
	useEffect(() => {
		...
		window.addEventListener('beforeunload', (e) => {
			...
			displayAlert(
				'WARNING: Attempting to close or refresh the tab is not allowed!'
			);
