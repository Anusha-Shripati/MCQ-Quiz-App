...
const Filters = () => {
  ...
  const { setCandidateFilter,setAssessmentOptions,setTechnologyOptions,assessmentOptions,technologyOptions} = useCandidateStore()


  useEffect(() => {
    const assessmentOptions= assessments?.data?.map((item: { id: string, name: string,technologies:{id:string,name:string}[] }) => ({ value: item.id, label: item.name,technologies:item.technologies })) || []
  ...
  const getData = ()=>{
    ...
    const payload: CandidateFilter = {
      ...
      technologyFilter: (formData.technologyFilter || []).map((item: TechnologyOption) => ({value:item.value,label:item.label})),
      assessmentFilter: (formData.assessmentFilter || []).map((item: AssessmentOption) => ({value:item.value,label:item.label})),
      created: formData.created?.range ? {
        days: "",
        range: {
          from: formData.created.range.from ? new Date(formData.created.range.from.setHours(0, 0, 0, 0)) : undefined,
          to: formData.created.range.to ? new Date(formData.created.range.to.setHours(23, 59, 59, 999)) : undefined
        }
      } : undefined
    };
