...
export class ResultService {
  ...
  list = async (params: ResultParams) => {
    ...
    const results = await this.prisma.results.findMany(query);
    const total = await this.prisma.results.count({ where });
  ...
  async updateScore(resultId: string, questionId: string, score: number) {
    try {
      ...
      const existingAns = await this.prisma.answers.findFirst({
      ...
      const result = await this.prisma.results.update({
        where: { id: resultId },
        data: {
          score: totalScore,
      ...
      const ans = await this.prisma.answers.update({
...
async getFeedback(resultId: string) {
  ...
  const candidateFeedback = await this.prisma.candidate_feedback.findFirst({
