...
export default function AssessmentDetails() {
  ...
  useEffect(() => {
    if (assessmentsData) {
      ...
      setCurrentPageEnd(
        Math.min(
          assessmentsData.data.page * itemsPerPage,
          assessmentsData.data.total
        )
      );
    }
  }, [assessmentsData]);
  ...
  const handlePerPageChange = (perPage: string) => {
    ...
    setCurrentPageEnd(
      Math.min(Number(perPage) * currentPage, assessments.length)
    );
    setCurrentPage(1);
  };
  if (editing) {
  ...
  return (
    <div className="p-4 bg-white dark:bg-[#334155] rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-200 hover:shadow-md">
      <StatusWrapper error={'error'} loading={isLoading} className="min-h-[500px]">
        <Pagination
