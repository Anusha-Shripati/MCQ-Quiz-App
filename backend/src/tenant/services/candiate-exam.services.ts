...
export class CandidateExamService {
  ...
  async getNextQuestion(examId: string, candidateId: string, currentQuestionId?: string) {
    ...
    const unansweredQuestions = await this.prisma.exam_questions.findMany({
      where: {
        ...whereClause,
        NOT: {
          question: {
  ...
  async submitAnswer(
    ...
    if (data.question_id) {
      const examQuestion = await this.prisma.exam_questions.findFirst({
        where: {
          exam_id: exam_id,
    ...
    const ans = await this.prisma.answers.findFirst({
      where: {
        exam_id: exam_id,
        question_id: data.question_id,
    ...
    if (ans) {
      const newAns = await this.prisma.answers.update({
        where: { id: ans.id },
        data: {
    ...
    const newAns = await this.prisma.answers.create({
      data: {
        exam_id: exam_id,
  ...
  async getExamStatus(examId: string, candidateId: string) {
    ...
    const answeredQuestions = await this.prisma.answers.count({
      where: {
        exam_id: examId,
        candidate_id: candidateId,
      },
  ...
  async finishExam(examId: string, candidateId: string) {
    try {
      ...
      const [, returnValue] = await this.prisma.$transaction([
        this.prisma.answers.updateMany({ where: { exam_id: examId }, data: { result_id: result.id } }),
  ...
  async resultCalculation(examId: string) {
    ...
    const allQuestions = await this.prisma.exam_questions.findMany({
      where: { exam_id: examId },
      include: { question: true },
    });
  ...
  async submitFeedback(
    ...
    try {
      ...
      const existingFeedback = await this.prisma.$queryRaw<{ id: string }[]>`
