...
export class UploadService {
  ...
  async mergeChunk(foldername: string, exam_id: string, body: MergeChunk, tenantId?: string) {
    if (storageMode === 's3') {
      return this.mergeChunkS3(foldername, exam_id, body, tenantId);
    } else {
      const path = await this.mergeChunkLocal(foldername, exam_id, tenantId);
  ...
  private async mergeChunkS3(filename: string, exam_id: string, body: MergeChunk, tenantId?: string) {
    try {
      const tenantFolder = tenantId || 'unknown';
      const command = new CompleteMultipartUploadCommand({
        Bucket: process.env.AWS_BUCKET_NAME,
        Key: `${tenantFolder}/${exam_id}/${filename}.mp4`,
      ...
      return {
        path: `${tenantFolder}/${exam_id}/${filename}.mp4`
      }
    } catch (error) {
      throw error;
    }
