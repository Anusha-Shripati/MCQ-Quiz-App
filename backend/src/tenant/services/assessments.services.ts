...
export class AssessmentsService {
  ...
  async getAssessments(filters: Filters) {
    ...
    const total = await this.prisma.assessments.count({
      where: { ...query },
    });
    const response =  {
      list: assessments,
  ...
  async createAssessments(data: AssessmentCreateInput): Promise<Assessments> {
    return this.prisma.assessments.create({ data });
  }

  async updateAssessments(id: string, data: AssessmentUpdateInput) {
    return this.prisma.assessments.update({ where: { id }, data });
  }
  async getAssessmentById(id: string) {
    const data = await this.cacheService.getKey(`assessment:${id}`);
    if(data) return JSON.parse(data)
    const response= this.prisma.assessments.findUnique({
      where: { id },
      include: {
        technologies: {
          where: { 
  ...
  async deleteAssessment(id: string): Promise<Assessments | null> {
    return this.prisma.assessments.update({ where: { id }, data: { deleted_at: new Date() } });
  }
  async assignTechnologiesToAssessment(assessmentId: string, technology: technologyPayload[]) {
    ...
    return this.prisma.assessment_technology.createMany({ data: assessment_technologies });
  }
  async deleteTechnologyAssessment(assessment_id: string) {
    return this.prisma.assessment_technology.deleteMany({
      where: { assessment_id }
    });
  }
