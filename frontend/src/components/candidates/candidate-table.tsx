...
function CandidateTable() {
	...
	const [selectedCandidate, setSelectedCandidate] =
		useState<null | CandidateFormData>(null);
	...
	useEffect(() => {
		...
		if (candidateFilter.technologyFilter.length > 0) {
			const techLabels = candidateFilter.technologyFilter.map(
				(item) => item.label
			);
		...
		if (candidateFilter.assessmentFilter.length > 0) {
			const assessmentLabels = candidateFilter.assessmentFilter.map(
				(item) => item.label
			);
	...
	useEffect(() => {
		...
		const filterParams = {
			...
			technologyFilter: params.get('technologyFilter')
				? JSON.parse(params.get('technologyFilter') as string).map(
						(label: string) => {
							const found = technologyOptions.find(
								(opt) => opt.label === label
							);
			...
			assessmentFilter: params.get('assessmentFilter')
				? JSON.parse(params.get('assessmentFilter') as string).map(
						(label: string) => {
							const found = assessmentOptions.find(
								(opt) => opt.label === label
							);
							return found || { value: '', label };
						}
	...
	const queryObj = useMemo(
		() => ({
			...
			created: candidateFilter.created
				? JSON.stringify(candidateFilter.created)
				: undefined,
		}),
							...
							onClick={async (e) => {
								...
								try {
									navigator.clipboard.writeText(
										candidate.meta.examLink as string
									);
									const expiresAt = new Date(
										candidate.meta.tokenExpiresAt as string
									);
									const formattedExpiration = expiresAt.toLocaleString();

									toast.success(
										`Exam link copied! Valid until ${formattedExpiration}`
									);
								} catch (error) {
