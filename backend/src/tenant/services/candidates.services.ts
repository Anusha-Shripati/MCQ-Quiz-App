...
export default class CandidatesService {
  ...
  constructor(prisma: PrismaClient){
    this.prisma = prisma;
    this.examService = new ExamService(prisma);
      ...
      const result = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (data.assessment_id && data.assessment_id !== existingCandidate.assessment_id) {
            ...
            await this.examService.createExamQuestionsForAssessment(
  ...
  async getCandidateById(id: string)  {
    try {
      ...
      if(data) return JSON.parse(data)
      const response = await this.prisma.candidate.findUnique({
        where: { id },
        include: {
          assessment: true,
          exam: true,
  ...
  async getCandidateByExamId(id: string) {
    ...
    const response=  await this.prisma.candidate.findFirst({
      where: { exam_id: id },
      select: {
        name: true,
        email: true,
