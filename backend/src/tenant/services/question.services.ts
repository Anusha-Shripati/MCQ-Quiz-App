...
export class QuestionService {
  ...
  async getQuestions(filters: {
    ...
    const response = this.prisma.questions.findMany({ include: { technology: true } });
  ...
  async getQuestionById(id: string): Promise<Questions | null> {
    ...
    const response = await this.prisma.questions.findUnique({ where: { id } });
  ...
  async getQuestionByName(question: string): Promise<Questions[] | null> {
    ...
    const response = await this.prisma.questions.findMany({ where: { question } });
  ...
  async deleteQuestion(questionId: string) {
    try {
      ...
      await this.prisma.exam_questions.deleteMany({
        where: { question_id: questionId },
      });
  ...
  async updateQuestion(id: string, data: QuestionsPayload) {
    return this.prisma.questions.update({ where: { id }, data });
  }
  ...
  async getQuestionByTechnologyId(filters: {
    ...
    const technology = await this.prisma.technology.findUnique({ where: { id: filters.technology_id } });
  ...
  async downloadQuestionFile(): Promise<Buffer> {
    const technologiesWithQuestions = await this.prisma.technology.findMany({
      take: 10,
      include: {
        questions: {
